...
def main():
    ...
    generate = st.sidebar.button("Generate Events Now")
    campaign_view = st.sidebar.button("View Attack Campaign Sample")
    st.sidebar.markdown("---")
    ...
    # --- Metrics / Analyst Scoreboard ---
    col1, col2, col3, col4, col5 = st.columns([2,2,2,2,2])
    col1.metric("Incidents", len(attack_logs))
    col2.metric("Blocked", int(defense_logs['defense_success'].sum()))
    col3.metric("Escalations", int(defense_logs['escalation_required'].sum()))
    mean_time = defense_logs['defense_response_time_sec'].dropna().mean()
    col4.metric("Avg. Response (s)", f"{mean_time:.1f}" if mean_time and not math.isnan(mean_time) else "-")
    # Analyst Score
    ...
    # --- Timeline plot + Escalation Markers ---
    attack_logs['dt'] = pd.to_datetime(attack_logs['timestamp'])
    attack_logs['end_dt'] = attack_logs['dt'] + pd.to_timedelta(5, unit='s')
    fig = px.timeline(
        attack_logs,
    ...
    # --- Recent Attacks, now clickable cards ---
    ...
    # --- Incident Deep Dive (shows details of the selected card) ---
